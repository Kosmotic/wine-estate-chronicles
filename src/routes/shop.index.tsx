...
export const Route = createFileRoute("/shop/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse our collection of organic, biodynamic and rare wines from the Valle d'Or estate." },
...
const types = ["All", "Red", "White", "Rosé", "Sparkling"] as const;
  ...
  const filtered = useMemo(
    () =>
      wines.filter((w) => {
        ...
        if (organic && !w.tags.some((t) => t.toLowerCase().includes("organic") || t.toLowerCase().includes("biodynamic"))) return false;
        if (query && !`${w.name} ${w.varietal} ${w.region}`.toLowerCase().includes(query.toLowerCase())) return false;
        return true;
      }),
    [type, max, organic, query]
  );

  return (
    <SiteLayout>
      <PageHero eyebrow="The Collection" title="Our Wines" subtitle="Six estate wines, made in vanishingly small quantities, with patience and intention." />
        ...
        <div className="container-luxe grid lg:grid-cols-[280px_1fr] gap-12">
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start space-y-10">
            <div>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-11 pl-10 pr-3 bg-cream border-b border-border focus:outline-none focus:border-burgundy transition"
                />
              </div>
            </div>
            <div>
                ...
                {types.map((t) => (
                  <button
                    ...
                    className={`block w-full text-left py-1.5 text-sm transition-colors ${
                      type === t ? "text-burgundy" : "text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    {type === t && <span className="text-gold mr-2">—</span>}
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full mt-4 accent-burgundy"
              />
          ...
          <div>
            <div className="flex justify-between items-end mb-8">
              ...
              <select className="bg-transparent border-b border-border text-sm py-1 pr-6 focus:outline-none">
                ...
                <Link key={w.slug} to="/shop/$slug" params={{ slug: w.slug }} className="group block">
                  <div className="relative aspect-[4/5] overflow-hidden bg-secondary">
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-1">
                      {w.tags.map((t) => (
                        <span key={t} className="bg-cream/95 text-burgundy text-[0.6rem] tracking-[0.18em] uppercase px-2 py-0.5">
                          {t}
                        </span>
                      ))}
                    </div>
                    <button
                      ...
                      className="absolute bottom-3 left-3 right-3 bg-burgundy text-cream py-3 text-[0.7rem] uppercase tracking-[0.22em] opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all"
                    >
                      Quick add — ${w.price}
                    </button>
                  </div>
            ...
            {filtered.length === 0 && (
              <p className="text-center py-20 text-muted-foreground">No wines match those filters.</p>
            )}
          </div>
        </div>
      </section>
