    ...
    <footer className="relative overflow-hidden bg-gradient-dark text-cream">
      ...
      <div className="container-luxe relative py-20">
        <div className="grid gap-10 md:grid-cols-2 border-b border-cream/15 pb-12">
          ...
          <form className="flex items-end gap-3 self-end w-full">
            <input
              type="email"
        ...
        <div className="grid gap-12 md:grid-cols-4 pt-14">
          <div>
            <Logo tone="light" />
            <p className="mt-6 text-sm text-cream/65 leading-relaxed max-w-xs font-script text-base">
              A family estate winery in the Valle d'Or, crafting wines of distinction since
              1987. Every bottle tells the story of our land, our hands, and our enduring
              patience.
            </p>
            <div className="mt-6 flex items-center gap-3">
              {[Instagram, Facebook, Youtube, Twitter].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="grid h-9 w-9 place-items-center rounded-full border border-cream/25 hover:border-gold hover:text-gold transition"
                  aria-label="Social"
                >
          ...
          <div>
            ...
            <p className="mt-5 text-burgundy bg-cream/95 inline-block px-3 py-1 text-sm">
              contact@whinarywines.com
            </p>
          </div>
        </div>

        <div className="mt-14 border-t border-cream/15 pt-6 flex flex-col md:flex-row gap-3 justify-between text-xs text-cream/55">
          <span>© 2026 Whinary Estate. All rights reserved. Please drink responsibly.</span>
          <span className="text-gold">whinarywines.com</span>
        </div>
      </div>
    </footer>
  );
  ...
  return (
    <div>
      ...
      <ul className="mt-5 space-y-3 text-cream/75">
        {links.map(([label, to]) => (
          <li key={label}>
            <Link to={to} className="hover:text-gold transition-colors text-sm">
              {label}
            </Link>
          </li>
        ))}
